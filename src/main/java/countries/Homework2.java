...
public class Homework2 {
    ...
    public Optional<String> streamPipeline8() {
        return countries.stream()
                .map(Country::getName)
                .max(Comparator.comparingInt(Homework2::vowelCount));
    }
        ...
        return Stream.of('a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v',
                ...
                .collect(Collectors
                        .toMap(character -> character,
                                character -> Long.valueOf(charCount(countries.stream()
                                        .map(Country::getName)
    ...
    public Map<ZoneId, Long> streamPipeline10() {
        return countries.stream().flatMap(country -> country.getTimezones().stream())
                ...
                .collect(Collectors.toMap(timezone -> timezone,
                        timezone -> countries.stream()
                                .flatMap(country -> country.getTimezones().stream())
                                .filter(timezone::equals).count()));
    }
    ...
    public Map<Region, Long> streamPipeline11() {
        return countries.stream()
                .collect(groupingBy(country -> country.getRegion(), filtering(country -> country.getName()
                        .toLowerCase()
                        .startsWith(country.getCode().toLowerCase()), counting())));
    }
    ...
    public Map<Boolean, Long> streamPipeline12() {
        var avgPopulation = countries.stream().
                mapToLong(Country::getPopulation).
                average().
                getAsDouble();

        return countries.stream()
                .collect(partitioningBy(country -> country.getPopulation() >= avgPopulation, counting()));
    }
    ...
    public Map<String, String> streamPipeline13() {
        return countries.stream()
                .collect(Collectors.toMap(Country::getCode, country -> country.getTranslations()
                        .get("pt")));
    }
    ...
    public Map<Region, List<String>> streamPipeline14() {
        return countries.stream().
                ...
                collect(groupingBy(Country::getRegion, mapping(Country::getCapital, toList())));
    }
    ...
    public Map<String, Double> streamPipeline15() {
        return countries.stream()
                .collect(Collectors.toMap(Country::getName, Homework2::populationDensity));
    }
